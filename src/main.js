import { createApp } from "vue"
import "./styles.css"
import App from "./App.vue"
import "bootstrap/dist/css/bootstrap.min.css"
import "bootstrap/dist/js/bootstrap.bundle.min.js"
import "bootstrap/dist/js/bootstrap.min.js"
import "layui/dist/css/layui.css"
import "layui/dist/layui.js"
import router from './router'
import { invoke } from '@tauri-apps/api'
import Database from "tauri-plugin-sql-api"
import { type } from "@tauri-apps/api/os"
import Vue3Dragscroll from 'vue3-dragscroll'
import tooltip from "./components/tooltip/directive"
import { BaseDirectory, writeTextFile, exists, createDir, writeBinaryFile } from '@tauri-apps/api/fs'
import { fetch as tfetch, Body, getClient } from "@tauri-apps/api/http"

const app = createApp(App)

const db_patch = await exists('database', { dir: BaseDirectory.App })
if (!db_patch) {
  await createDir('database', { dir: BaseDirectory.App, recursive: true })
  const db_file = await exists('database/database', { dir: BaseDirectory.App })
  if (!db_file) {
    await writeBinaryFile('database/database', new Uint8Array(), { dir: BaseDirectory.App })
    const db = await Database.load("sqlite:./database/database")
    db.execute(`CREATE TABLE games (
      id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
      title TEXT(255) NOT NULL
    , game_status INTEGER DEFAULT (0) NOT NULL, group_nums INTEGER DEFAULT (0) NOT NULL, game_format INTEGER DEFAULT (1) NOT NULL, game_sort INTEGER DEFAULT (0), group_wins INTEGER DEFAULT (1));
    
    CREATE INDEX games_game_sort_IDX ON games (game_sort);
    
    CREATE TABLE "matchs" (
      id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
      game_id INTEGER DEFAULT (0) NOT NULL,
      player_one_id INTEGER DEFAULT (0) NOT NULL,
      player_two_id INTEGER DEFAULT (0) NOT NULL,
      player_one_score INTEGER DEFAULT (0) NOT NULL,
      player_two_score INTEGER DEFAULT (0) NOT NULL,
      winner INTEGER DEFAULT (0) NOT NULL,
      match_status INTEGER DEFAULT (0) NOT NULL,
      parent_id_1 INTEGER DEFAULT (0) NOT NULL,
      parent_id_2 INTEGER DEFAULT (0) NOT NULL,
      match_group INTEGER DEFAULT (0) NOT NULL,
      match_level INTEGER DEFAULT (0) NOT NULL,
      match_type INTEGER DEFAULT (1) NOT NULL,
      change_user_place INTEGER DEFAULT (0),
      group_sort INTEGER DEFAULT (0) NOT NULL
    , is_final INTEGER DEFAULT (0));
    
    CREATE INDEX matchs_player_one_id_IDX ON matchs (player_one_id);
    CREATE INDEX matchs_player_two_id_IDX ON matchs (player_two_id);
    CREATE INDEX matchs_game_id_IDX ON matchs (game_id);
    CREATE INDEX matchs_parent_id_1_IDX ON matchs (parent_id_1);
    CREATE INDEX matchs_parent_id_2_IDX ON matchs (parent_id_2);
    CREATE INDEX matchs_match_group_IDX ON matchs (match_group);
    CREATE INDEX matchs_match_level_IDX ON matchs (match_level);
    CREATE INDEX matchs_match_type_IDX ON matchs (match_type);
    CREATE INDEX matchs_group_sort_IDX ON matchs (group_sort);
    CREATE INDEX matchs_is_final_IDX ON matchs (is_final);
    CREATE INDEX matchs_winner_IDX ON matchs (winner);
    
    CREATE TABLE "players" (
      id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
      game_id INTEGER NOT NULL,
      title TEXT(255) NOT NULL,
      sort_num INTEGER DEFAULT (0)
    , fast_copy TEXT);
    
    CREATE INDEX player_games_id_IDX ON "players" (game_id);
    CREATE INDEX player_sort_num_IDX ON "players" (sort_num);`)
  }
}

let base_dir = await exists('obs', { dir: BaseDirectory.App })
if (!base_dir) {
  await createDir('obs', { dir: BaseDirectory.App, recursive: true })
}
let file_game_name = await exists('obs/game_name.txt', { dir: BaseDirectory.App })
if (!file_game_name) {
  await writeTextFile('obs/game_name.txt', '', { dir: BaseDirectory.App })
}
let p1_title = await exists('obs/p1_title.txt', { dir: BaseDirectory.App })
if (!p1_title) {
  await writeTextFile('obs/p1_title.txt', '', { dir: BaseDirectory.App })
}
let p2_title = await exists('obs/p2_title.txt', { dir: BaseDirectory.App })
if (!p2_title) {
  await writeTextFile('obs/p2_title.txt', '', { dir: BaseDirectory.App })
}
let p1_score = await exists('obs/p1_score.txt', { dir: BaseDirectory.App })
if (!p1_score) {
  await writeTextFile('obs/p1_score.txt', '0', { dir: BaseDirectory.App })
}
let p2_score = await exists('obs/p2_score.txt', { dir: BaseDirectory.App })
if (!p2_score) {
  await writeTextFile('obs/p2_score.txt', '0', { dir: BaseDirectory.App })
}
let match = await exists('obs/match.txt', { dir: BaseDirectory.App })
if (!match) {
  await writeTextFile('obs/match.txt', '', { dir: BaseDirectory.App })
}
let p1chountry = await exists('obs/p1country.html', { dir: BaseDirectory.App })
if (!p1chountry) {
  await writeTextFile('obs/p1country.html', `
    <img src="p1.svg" id="liveImage" class="flag-icon" width="100%" />
    <script>
    const img = document.getElementById('liveImage');

    function updateImage() {
        // 通过添加随机时间戳后缀，强制浏览器跳过缓存重新读取文件
        img.src = 'p1.svg?t=' + new Date().getTime();
    }

    // 每 500 毫秒检查一次（或者根据你生成的频率调整）
    setInterval(updateImage, 500);
    </script>
  `, { dir: BaseDirectory.App })
}
let p2chountry = await exists('obs/p2country.html', { dir: BaseDirectory.App })
if (!p2chountry) {
  await writeTextFile('obs/p2country.html', `
    <img src="p2.svg" id="liveImage" class="flag-icon" width="100%" />
    <script>
    const img = document.getElementById('liveImage');

    function updateImage() {
        // 通过添加随机时间戳后缀，强制浏览器跳过缓存重新读取文件
        img.src = 'p2.svg?t=' + new Date().getTime();
    }

    // 每 500 毫秒检查一次（或者根据你生成的频率调整）
    setInterval(updateImage, 500);
    </script>
  `, { dir: BaseDirectory.App })
}
if (import.meta.env.VITE_DEBUG) {
  app.config.globalProperties.$debug = true
} else {
  app.config.globalProperties.$debug = false
}
app.config.globalProperties.$createDb = async () => {
  try {
    const db = await Database.load("sqlite:./database/database")
    return db
  } catch (error) {
    console.error(error)
  }
}


app.config.globalProperties.$db = await app.config.globalProperties.$createDb()
app.config.globalProperties.$pageSize = 20

// 判断更新表是否存在，不存在创建
let update_table = await app.config.globalProperties.$db.select(`SELECT name FROM sqlite_master WHERE type='table' AND name='update_table'`);
if (update_table.length == 0) {
  await app.config.globalProperties.$db.execute(`CREATE TABLE update_table (
    id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
    version_1 INTEGER NOT NULL,
    version_2 INTEGER NOT NULL,
    version_3 INTEGER NOT NULL
  );
  
  CREATE INDEX update_table_version_1_IDX ON update_table (version_1);
  CREATE INDEX update_table_version_2_IDX ON update_table (version_2);
  CREATE INDEX update_table_version_3_IDX ON update_table (version_3);`)
}
import update from './update/sql.json' assert { type: 'JSON' }
for (let k in update) {
  const version = k.split('.')
  let updated = await app.config.globalProperties.$db.select(`select id from update_table where version_1 = ${version[0]} and version_2 = ${version[1]} and version_3 = ${version[2]}`)
  if (updated.length == 0) {
    for (let k1 = 0; k1 < update[k].length; k1++) {
      let need_update = true
      if (update[k][k1].prepare.length > 0) {
        let prepare = await app.config.globalProperties.$db.select(update[k][k1].prepare)
        if (prepare.length > 0) {
          need_update = false
        }
      }
      if (need_update) {
        await app.config.globalProperties.$db.execute(update[k][k1].update)
      }
    }
    await app.config.globalProperties.$db.execute(`insert into update_table (version_1, version_2, version_3) values(${version[0]}, ${version[1]}, ${version[2]})`)
  }
}

app.config.globalProperties.$page = async (table, page, pageSize, where, fields, order) => {
  if (!pageSize || isNaN(pageSize) || pageSize <= 0 || pageSize > 200) {
    pageSize = app.config.globalProperties.$pageSize
  }
  if (!page || isNaN(page) || page < 1) {
    page = 1
  }
  let whereStr = ``
  if (where) {
    whereStr = ` WHERE ${where}`
  }
  let result = await app.config.globalProperties.$db.select(`SELECT COUNT(*) AS mycount FROM ${table} ${whereStr}`)
  let maxPage = 1, count = 0
  if (result.length > 0) {
    count = result[0].mycount
    maxPage = Math.ceil(count / pageSize)
  }
  const limit = ` LIMIT ${pageSize} OFFSET ${(page - 1) * pageSize}`
  if (!order) {
    order = ' ORDER BY id DESC'
  } else {
    order = ` ORDER BY ${order}`
  }
  if (!fields) {
    fields = '*'
  }
  result = await app.config.globalProperties.$db.select(`SELECT ${fields} FROM ${table} ${whereStr} ${order} ${limit}`)
  return {
    page: page,
    pageSize: pageSize,
    count: count,
    maxPage: maxPage,
    data: result,
  }
}

app.config.globalProperties.$setPageSize = (pageSize) => {
  if (!pageSize || isNaN(pageSize) || pageSize <= 0 || pageSize > 200) {
    pageSize = 200
  }
  localStorage.setItem('pageSize', pageSize)
}
if (!localStorage.getItem('pageSize')) {
  app.config.globalProperties.$setPageSize()
}

app.config.globalProperties.$getGame = async (id) => {
  if (!id || isNaN(id) || id <= 0) {
    return '比赛id错误'
  }
  let result = await app.config.globalProperties.$db.select('SELECT * FROM games WHERE id = ?', [id])
  if (result.length > 0) {
    return result[0]
  } else {
    return '比赛id不存在'
  }
}

app.config.globalProperties.$tfetch = async (url = "", data = {}) => {
  const index = layer.load()
  let formdata = new FormData()
  for (let key in data) {
    formdata.append(key, data[key])
  }
  const body = Body.form(formdata)
  try {
    let res = await tfetch(url, {
      method: "POST",
      timeout: 5,
      body: body,
    })
    layer.close(index)
    return res.data
  } catch (error) {
    console.log(error)
    return { code: -1, msg: '网络错误' }
  }
}

app.config.globalProperties.$sendShare = async (data) => {
  if (!localStorage.getItem('config')) {
    return false
  }
  let config = JSON.parse(localStorage.getItem('config'))
  if (!config.shareApi) {
    return false
  }
  if (config.shareApi.url.length == 0 || config.shareApi.key.length == 0) {
    return false
  }
  data.action = 'update'
  data.key = config.shareApi.key
  await app.config.globalProperties.$tfetch(config.shareApi.url, data)
}

app.config.globalProperties.$country = {
  "1": {
    "中文名称": "阿富汗",
    "英文短名称": "Afghanistan",
    "二位代码": "AF",
    "三位代码": "AFG",
    "数字代码": "4",
    "ISO.3166-2": "ISO 3166-2:AF",
    "ISO.3166上标为独立主权": "是"
  },
  "2": {
    "中文名称": "奥兰",
    "英文短名称": "Åland Islands",
    "二位代码": "AX",
    "三位代码": "ALA",
    "数字代码": "248",
    "ISO.3166-2": "ISO 3166-2:AX",
    "ISO.3166上标为独立主权": "否"
  },
  "3": {
    "中文名称": "阿尔巴尼亚",
    "英文短名称": "Albania",
    "二位代码": "AL",
    "三位代码": "ALB",
    "数字代码": "8",
    "ISO.3166-2": "ISO 3166-2:AL",
    "ISO.3166上标为独立主权": "是"
  },
  "4": {
    "中文名称": "阿尔及利亚",
    "英文短名称": "Algeria",
    "二位代码": "DZ",
    "三位代码": "DZA",
    "数字代码": "12",
    "ISO.3166-2": "ISO 3166-2:DZ",
    "ISO.3166上标为独立主权": "是"
  },
  "5": {
    "中文名称": "美属萨摩亚",
    "英文短名称": "American Samoa",
    "二位代码": "AS",
    "三位代码": "ASM",
    "数字代码": "16",
    "ISO.3166-2": "ISO 3166-2:AS",
    "ISO.3166上标为独立主权": "否"
  },
  "6": {
    "中文名称": "安道尔",
    "英文短名称": "Andorra",
    "二位代码": "AD",
    "三位代码": "AND",
    "数字代码": "20",
    "ISO.3166-2": "ISO 3166-2:AD",
    "ISO.3166上标为独立主权": "是"
  },
  "7": {
    "中文名称": "安哥拉",
    "英文短名称": "Angola",
    "二位代码": "AO",
    "三位代码": "AGO",
    "数字代码": "24",
    "ISO.3166-2": "ISO 3166-2:AO",
    "ISO.3166上标为独立主权": "是"
  },
  "8": {
    "中文名称": "安圭拉",
    "英文短名称": "Anguilla",
    "二位代码": "AI",
    "三位代码": "AIA",
    "数字代码": "660",
    "ISO.3166-2": "ISO 3166-2:AI",
    "ISO.3166上标为独立主权": "否"
  },
  "9": {
    "中文名称": "南极洲",
    "英文短名称": "Antarctica",
    "二位代码": "AQ",
    "三位代码": "ATA",
    "数字代码": "10",
    "ISO.3166-2": "ISO 3166-2:AQ",
    "ISO.3166上标为独立主权": "否"
  },
  "10": {
    "中文名称": "安提瓜和巴布达",
    "英文短名称": "Antigua and Barbuda",
    "二位代码": "AG",
    "三位代码": "ATG",
    "数字代码": "28",
    "ISO.3166-2": "ISO 3166-2:AG",
    "ISO.3166上标为独立主权": "是"
  },
  "11": {
    "中文名称": "阿根廷",
    "英文短名称": "Argentina",
    "二位代码": "AR",
    "三位代码": "ARG",
    "数字代码": "32",
    "ISO.3166-2": "ISO 3166-2:AR",
    "ISO.3166上标为独立主权": "是"
  },
  "12": {
    "中文名称": "亚美尼亚",
    "英文短名称": "Armenia",
    "二位代码": "AM",
    "三位代码": "ARM",
    "数字代码": "51",
    "ISO.3166-2": "ISO 3166-2:AM",
    "ISO.3166上标为独立主权": "是"
  },
  "13": {
    "中文名称": "阿鲁巴",
    "英文短名称": "Aruba",
    "二位代码": "AW",
    "三位代码": "ABW",
    "数字代码": "533",
    "ISO.3166-2": "ISO 3166-2:AW",
    "ISO.3166上标为独立主权": "否"
  },
  "14": {
    "中文名称": "澳大利亚",
    "英文短名称": "Australia",
    "二位代码": "AU",
    "三位代码": "AUS",
    "数字代码": "36",
    "ISO.3166-2": "ISO 3166-2:AU",
    "ISO.3166上标为独立主权": "是"
  },
  "15": {
    "中文名称": "奥地利",
    "英文短名称": "Austria",
    "二位代码": "AT",
    "三位代码": "AUT",
    "数字代码": "40",
    "ISO.3166-2": "ISO 3166-2:AT",
    "ISO.3166上标为独立主权": "是"
  },
  "16": {
    "中文名称": "阿塞拜疆",
    "英文短名称": "Azerbaijan",
    "二位代码": "AZ",
    "三位代码": "AZE",
    "数字代码": "31",
    "ISO.3166-2": "ISO 3166-2:AZ",
    "ISO.3166上标为独立主权": "是"
  },
  "17": {
    "中文名称": "巴哈马",
    "英文短名称": "Bahamas",
    "二位代码": "BS",
    "三位代码": "BHS",
    "数字代码": "44",
    "ISO.3166-2": "ISO 3166-2:BS",
    "ISO.3166上标为独立主权": "是"
  },
  "18": {
    "中文名称": "巴林",
    "英文短名称": "Bahrain",
    "二位代码": "BH",
    "三位代码": "BHR",
    "数字代码": "48",
    "ISO.3166-2": "ISO 3166-2:BH",
    "ISO.3166上标为独立主权": "是"
  },
  "19": {
    "中文名称": "孟加拉国",
    "英文短名称": "Bangladesh",
    "二位代码": "BD",
    "三位代码": "BGD",
    "数字代码": "50",
    "ISO.3166-2": "ISO 3166-2:BD",
    "ISO.3166上标为独立主权": "是"
  },
  "20": {
    "中文名称": "巴巴多斯",
    "英文短名称": "Barbados",
    "二位代码": "BB",
    "三位代码": "BRB",
    "数字代码": "52",
    "ISO.3166-2": "ISO 3166-2:BB",
    "ISO.3166上标为独立主权": "是"
  },
  "21": {
    "中文名称": "白俄罗斯",
    "英文短名称": "Belarus",
    "二位代码": "BY",
    "三位代码": "BLR",
    "数字代码": "112",
    "ISO.3166-2": "ISO 3166-2:BY",
    "ISO.3166上标为独立主权": "是"
  },
  "22": {
    "中文名称": "比利时",
    "英文短名称": "Belgium",
    "二位代码": "BE",
    "三位代码": "BEL",
    "数字代码": "56",
    "ISO.3166-2": "ISO 3166-2:BE",
    "ISO.3166上标为独立主权": "是"
  },
  "23": {
    "中文名称": "伯利兹",
    "英文短名称": "Belize",
    "二位代码": "BZ",
    "三位代码": "BLZ",
    "数字代码": "84",
    "ISO.3166-2": "ISO 3166-2:BZ",
    "ISO.3166上标为独立主权": "是"
  },
  "24": {
    "中文名称": "贝宁",
    "英文短名称": "Benin",
    "二位代码": "BJ",
    "三位代码": "BEN",
    "数字代码": "204",
    "ISO.3166-2": "ISO 3166-2:BJ",
    "ISO.3166上标为独立主权": "是"
  },
  "25": {
    "中文名称": "百慕大",
    "英文短名称": "Bermuda",
    "二位代码": "BM",
    "三位代码": "BMU",
    "数字代码": "60",
    "ISO.3166-2": "ISO 3166-2:BM",
    "ISO.3166上标为独立主权": "否"
  },
  "26": {
    "中文名称": "不丹",
    "英文短名称": "Bhutan",
    "二位代码": "BT",
    "三位代码": "BTN",
    "数字代码": "64",
    "ISO.3166-2": "ISO 3166-2:BT",
    "ISO.3166上标为独立主权": "是"
  },
  "27": {
    "中文名称": "玻利维亚",
    "英文短名称": "Bolivia (Plurinational State of)",
    "二位代码": "BO",
    "三位代码": "BOL",
    "数字代码": "68",
    "ISO.3166-2": "ISO 3166-2:BO",
    "ISO.3166上标为独立主权": "是"
  },
  "28": {
    "中文名称": "荷兰加勒比区",
    "英文短名称": "Bonaire, Sint Eustatius and Saba",
    "二位代码": "BQ",
    "三位代码": "BES",
    "数字代码": "535",
    "ISO.3166-2": "ISO 3166-2:BQ",
    "ISO.3166上标为独立主权": "否"
  },
  "29": {
    "中文名称": "波黑",
    "英文短名称": "Bosnia and Herzegovina",
    "二位代码": "BA",
    "三位代码": "BIH",
    "数字代码": "70",
    "ISO.3166-2": "ISO 3166-2:BA",
    "ISO.3166上标为独立主权": "是"
  },
  "30": {
    "中文名称": "博茨瓦纳",
    "英文短名称": "Botswana",
    "二位代码": "BW",
    "三位代码": "BWA",
    "数字代码": "72",
    "ISO.3166-2": "ISO 3166-2:BW",
    "ISO.3166上标为独立主权": "是"
  },
  "31": {
    "中文名称": "布韦岛",
    "英文短名称": "Bouvet Island",
    "二位代码": "BV",
    "三位代码": "BVT",
    "数字代码": "74",
    "ISO.3166-2": "ISO 3166-2:BV",
    "ISO.3166上标为独立主权": "否"
  },
  "32": {
    "中文名称": "巴西",
    "英文短名称": "Brazil",
    "二位代码": "BR",
    "三位代码": "BRA",
    "数字代码": "76",
    "ISO.3166-2": "ISO 3166-2:BR",
    "ISO.3166上标为独立主权": "是"
  },
  "33": {
    "中文名称": "英属印度洋领地",
    "英文短名称": "British Indian Ocean Territory",
    "二位代码": "IO",
    "三位代码": "IOT",
    "数字代码": "86",
    "ISO.3166-2": "ISO 3166-2:IO",
    "ISO.3166上标为独立主权": "否"
  },
  "34": {
    "中文名称": "文莱",
    "英文短名称": "Brunei Darussalam",
    "二位代码": "BN",
    "三位代码": "BRN",
    "数字代码": "96",
    "ISO.3166-2": "ISO 3166-2:BN",
    "ISO.3166上标为独立主权": "是"
  },
  "35": {
    "中文名称": "保加利亚",
    "英文短名称": "Bulgaria",
    "二位代码": "BG",
    "三位代码": "BGR",
    "数字代码": "100",
    "ISO.3166-2": "ISO 3166-2:BG",
    "ISO.3166上标为独立主权": "是"
  },
  "36": {
    "中文名称": "布基纳法索",
    "英文短名称": "Burkina Faso",
    "二位代码": "BF",
    "三位代码": "BFA",
    "数字代码": "854",
    "ISO.3166-2": "ISO 3166-2:BF",
    "ISO.3166上标为独立主权": "是"
  },
  "37": {
    "中文名称": "布隆迪",
    "英文短名称": "Burundi",
    "二位代码": "BI",
    "三位代码": "BDI",
    "数字代码": "108",
    "ISO.3166-2": "ISO 3166-2:BI",
    "ISO.3166上标为独立主权": "是"
  },
  "38": {
    "中文名称": "佛得角",
    "英文短名称": "Cabo Verde",
    "二位代码": "CV",
    "三位代码": "CPV",
    "数字代码": "132",
    "ISO.3166-2": "ISO 3166-2:CV",
    "ISO.3166上标为独立主权": "是"
  },
  "39": {
    "中文名称": "柬埔寨",
    "英文短名称": "Cambodia",
    "二位代码": "KH",
    "三位代码": "KHM",
    "数字代码": "116",
    "ISO.3166-2": "ISO 3166-2:KH",
    "ISO.3166上标为独立主权": "是"
  },
  "40": {
    "中文名称": "喀麦隆",
    "英文短名称": "Cameroon",
    "二位代码": "CM",
    "三位代码": "CMR",
    "数字代码": "120",
    "ISO.3166-2": "ISO 3166-2:CM",
    "ISO.3166上标为独立主权": "是"
  },
  "41": {
    "中文名称": "加拿大",
    "英文短名称": "Canada",
    "二位代码": "CA",
    "三位代码": "CAN",
    "数字代码": "124",
    "ISO.3166-2": "ISO 3166-2:CA",
    "ISO.3166上标为独立主权": "是"
  },
  "42": {
    "中文名称": "开曼群岛",
    "英文短名称": "Cayman Islands",
    "二位代码": "KY",
    "三位代码": "CYM",
    "数字代码": "136",
    "ISO.3166-2": "ISO 3166-2:KY",
    "ISO.3166上标为独立主权": "否"
  },
  "43": {
    "中文名称": "中非",
    "英文短名称": "Central African Republic",
    "二位代码": "CF",
    "三位代码": "CAF",
    "数字代码": "140",
    "ISO.3166-2": "ISO 3166-2:CF",
    "ISO.3166上标为独立主权": "是"
  },
  "44": {
    "中文名称": "乍得",
    "英文短名称": "Chad",
    "二位代码": "TD",
    "三位代码": "TCD",
    "数字代码": "148",
    "ISO.3166-2": "ISO 3166-2:TD",
    "ISO.3166上标为独立主权": "是"
  },
  "45": {
    "中文名称": "智利",
    "英文短名称": "Chile",
    "二位代码": "CL",
    "三位代码": "CHL",
    "数字代码": "152",
    "ISO.3166-2": "ISO 3166-2:CL",
    "ISO.3166上标为独立主权": "是"
  },
  "46": {
    "中文名称": "中国",
    "英文短名称": "China",
    "二位代码": "CN",
    "三位代码": "CHN",
    "数字代码": "156",
    "ISO.3166-2": "ISO 3166-2:CN",
    "ISO.3166上标为独立主权": "是"
  },
  "47": {
    "中文名称": "圣诞岛",
    "英文短名称": "Christmas Island",
    "二位代码": "CX",
    "三位代码": "CXR",
    "数字代码": "162",
    "ISO.3166-2": "ISO 3166-2:CX",
    "ISO.3166上标为独立主权": "否"
  },
  "48": {
    "中文名称": "科科斯（基林）群岛",
    "英文短名称": "Cocos (Keeling) Islands",
    "二位代码": "CC",
    "三位代码": "CCK",
    "数字代码": "166",
    "ISO.3166-2": "ISO 3166-2:CC",
    "ISO.3166上标为独立主权": "否"
  },
  "49": {
    "中文名称": "哥伦比亚",
    "英文短名称": "Colombia",
    "二位代码": "CO",
    "三位代码": "COL",
    "数字代码": "170",
    "ISO.3166-2": "ISO 3166-2:CO",
    "ISO.3166上标为独立主权": "是"
  },
  "50": {
    "中文名称": "科摩罗",
    "英文短名称": "Comoros",
    "二位代码": "KM",
    "三位代码": "COM",
    "数字代码": "174",
    "ISO.3166-2": "ISO 3166-2:KM",
    "ISO.3166上标为独立主权": "是"
  },
  "51": {
    "中文名称": "刚果共和国",
    "英文短名称": "Congo",
    "二位代码": "CG",
    "三位代码": "COG",
    "数字代码": "178",
    "ISO.3166-2": "ISO 3166-2:CG",
    "ISO.3166上标为独立主权": "是"
  },
  "52": {
    "中文名称": "刚果民主共和国",
    "英文短名称": "Congo (Democratic Republic of the)",
    "二位代码": "CD",
    "三位代码": "COD",
    "数字代码": "180",
    "ISO.3166-2": "ISO 3166-2:CD",
    "ISO.3166上标为独立主权": "是"
  },
  "53": {
    "中文名称": "库克群岛",
    "英文短名称": "Cook Islands",
    "二位代码": "CK",
    "三位代码": "COK",
    "数字代码": "184",
    "ISO.3166-2": "ISO 3166-2:CK",
    "ISO.3166上标为独立主权": "否"
  },
  "54": {
    "中文名称": "哥斯达黎加",
    "英文短名称": "Costa Rica",
    "二位代码": "CR",
    "三位代码": "CRI",
    "数字代码": "188",
    "ISO.3166-2": "ISO 3166-2:CR",
    "ISO.3166上标为独立主权": "是"
  },
  "55": {
    "中文名称": "科特迪瓦",
    "英文短名称": "C?te d'Ivoire",
    "二位代码": "CI",
    "三位代码": "CIV",
    "数字代码": "384",
    "ISO.3166-2": "ISO 3166-2:CI",
    "ISO.3166上标为独立主权": "是"
  },
  "56": {
    "中文名称": "克罗地亚",
    "英文短名称": "Croatia",
    "二位代码": "HR",
    "三位代码": "HRV",
    "数字代码": "191",
    "ISO.3166-2": "ISO 3166-2:HR",
    "ISO.3166上标为独立主权": "是"
  },
  "57": {
    "中文名称": "古巴",
    "英文短名称": "Cuba",
    "二位代码": "CU",
    "三位代码": "CUB",
    "数字代码": "192",
    "ISO.3166-2": "ISO 3166-2:CU",
    "ISO.3166上标为独立主权": "是"
  },
  "58": {
    "中文名称": "库拉索",
    "英文短名称": "Cura?ao",
    "二位代码": "CW",
    "三位代码": "CUW",
    "数字代码": "531",
    "ISO.3166-2": "ISO 3166-2:CW",
    "ISO.3166上标为独立主权": "否"
  },
  "59": {
    "中文名称": "塞浦路斯",
    "英文短名称": "Cyprus",
    "二位代码": "CY",
    "三位代码": "CYP",
    "数字代码": "196",
    "ISO.3166-2": "ISO 3166-2:CY",
    "ISO.3166上标为独立主权": "是"
  },
  "60": {
    "中文名称": "捷克",
    "英文短名称": "Czechia",
    "二位代码": "CZ",
    "三位代码": "CZE",
    "数字代码": "203",
    "ISO.3166-2": "ISO 3166-2:CZ",
    "ISO.3166上标为独立主权": "是"
  },
  "61": {
    "中文名称": "丹麦",
    "英文短名称": "Denmark",
    "二位代码": "DK",
    "三位代码": "DNK",
    "数字代码": "208",
    "ISO.3166-2": "ISO 3166-2:DK",
    "ISO.3166上标为独立主权": "是"
  },
  "62": {
    "中文名称": "吉布提",
    "英文短名称": "Djibouti",
    "二位代码": "DJ",
    "三位代码": "DJI",
    "数字代码": "262",
    "ISO.3166-2": "ISO 3166-2:DJ",
    "ISO.3166上标为独立主权": "是"
  },
  "63": {
    "中文名称": "多米尼克",
    "英文短名称": "Dominica",
    "二位代码": "DM",
    "三位代码": "DMA",
    "数字代码": "212",
    "ISO.3166-2": "ISO 3166-2:DM",
    "ISO.3166上标为独立主权": "是"
  },
  "64": {
    "中文名称": "多米尼加",
    "英文短名称": "Dominican Republic",
    "二位代码": "DO",
    "三位代码": "DOM",
    "数字代码": "214",
    "ISO.3166-2": "ISO 3166-2:DO",
    "ISO.3166上标为独立主权": "是"
  },
  "65": {
    "中文名称": "厄瓜多尔",
    "英文短名称": "Ecuador",
    "二位代码": "EC",
    "三位代码": "ECU",
    "数字代码": "218",
    "ISO.3166-2": "ISO 3166-2:EC",
    "ISO.3166上标为独立主权": "是"
  },
  "66": {
    "中文名称": "埃及",
    "英文短名称": "Egypt",
    "二位代码": "EG",
    "三位代码": "EGY",
    "数字代码": "818",
    "ISO.3166-2": "ISO 3166-2:EG",
    "ISO.3166上标为独立主权": "是"
  },
  "67": {
    "中文名称": "萨尔瓦多",
    "英文短名称": "El Salvador",
    "二位代码": "SV",
    "三位代码": "SLV",
    "数字代码": "222",
    "ISO.3166-2": "ISO 3166-2:SV",
    "ISO.3166上标为独立主权": "是"
  },
  "68": {
    "中文名称": "赤道几内亚",
    "英文短名称": "Equatorial Guinea",
    "二位代码": "GQ",
    "三位代码": "GNQ",
    "数字代码": "226",
    "ISO.3166-2": "ISO 3166-2:GQ",
    "ISO.3166上标为独立主权": "是"
  },
  "69": {
    "中文名称": "厄立特里亚",
    "英文短名称": "Eritrea",
    "二位代码": "ER",
    "三位代码": "ERI",
    "数字代码": "232",
    "ISO.3166-2": "ISO 3166-2:ER",
    "ISO.3166上标为独立主权": "是"
  },
  "70": {
    "中文名称": "爱沙尼亚",
    "英文短名称": "Estonia",
    "二位代码": "EE",
    "三位代码": "EST",
    "数字代码": "233",
    "ISO.3166-2": "ISO 3166-2:EE",
    "ISO.3166上标为独立主权": "是"
  },
  "71": {
    "中文名称": "斯威士兰",
    "英文短名称": "Eswatini",
    "二位代码": "SZ",
    "三位代码": "SWZ",
    "数字代码": "748",
    "ISO.3166-2": "ISO 3166-2:SZ",
    "ISO.3166上标为独立主权": "是"
  },
  "72": {
    "中文名称": "埃塞俄比亚",
    "英文短名称": "Ethiopia",
    "二位代码": "ET",
    "三位代码": "ETH",
    "数字代码": "231",
    "ISO.3166-2": "ISO 3166-2:ET",
    "ISO.3166上标为独立主权": "是"
  },
  "73": {
    "中文名称": "福克兰群岛",
    "英文短名称": "Falkland Islands (Malvinas)",
    "二位代码": "FK",
    "三位代码": "FLK",
    "数字代码": "238",
    "ISO.3166-2": "ISO 3166-2:FK",
    "ISO.3166上标为独立主权": "否"
  },
  "74": {
    "中文名称": "法罗群岛",
    "英文短名称": "Faroe Islands",
    "二位代码": "FO",
    "三位代码": "FRO",
    "数字代码": "234",
    "ISO.3166-2": "ISO 3166-2:FO",
    "ISO.3166上标为独立主权": "否"
  },
  "75": {
    "中文名称": "斐济",
    "英文短名称": "Fiji",
    "二位代码": "FJ",
    "三位代码": "FJI",
    "数字代码": "242",
    "ISO.3166-2": "ISO 3166-2:FJ",
    "ISO.3166上标为独立主权": "是"
  },
  "76": {
    "中文名称": "芬兰",
    "英文短名称": "Finland",
    "二位代码": "FI",
    "三位代码": "FIN",
    "数字代码": "246",
    "ISO.3166-2": "ISO 3166-2:FI",
    "ISO.3166上标为独立主权": "是"
  },
  "77": {
    "中文名称": "法国",
    "英文短名称": "France",
    "二位代码": "FR",
    "三位代码": "FRA",
    "数字代码": "250",
    "ISO.3166-2": "ISO 3166-2:FR",
    "ISO.3166上标为独立主权": "是"
  },
  "78": {
    "中文名称": "法属圭亚那",
    "英文短名称": "French Guiana",
    "二位代码": "GF",
    "三位代码": "GUF",
    "数字代码": "254",
    "ISO.3166-2": "ISO 3166-2:GF",
    "ISO.3166上标为独立主权": "否"
  },
  "79": {
    "中文名称": "法属波利尼西亚",
    "英文短名称": "French Polynesia",
    "二位代码": "PF",
    "三位代码": "PYF",
    "数字代码": "258",
    "ISO.3166-2": "ISO 3166-2:PF",
    "ISO.3166上标为独立主权": "否"
  },
  "80": {
    "中文名称": "法属南部和南极领地",
    "英文短名称": "French Southern Territories",
    "二位代码": "TF",
    "三位代码": "ATF",
    "数字代码": "260",
    "ISO.3166-2": "ISO 3166-2:TF",
    "ISO.3166上标为独立主权": "否"
  },
  "81": {
    "中文名称": "加蓬",
    "英文短名称": "Gabon",
    "二位代码": "GA",
    "三位代码": "GAB",
    "数字代码": "266",
    "ISO.3166-2": "ISO 3166-2:GA",
    "ISO.3166上标为独立主权": "是"
  },
  "82": {
    "中文名称": "冈比亚",
    "英文短名称": "Gambia",
    "二位代码": "GM",
    "三位代码": "GMB",
    "数字代码": "270",
    "ISO.3166-2": "ISO 3166-2:GM",
    "ISO.3166上标为独立主权": "是"
  },
  "83": {
    "中文名称": "格鲁吉亚",
    "英文短名称": "Georgia",
    "二位代码": "GE",
    "三位代码": "GEO",
    "数字代码": "268",
    "ISO.3166-2": "ISO 3166-2:GE",
    "ISO.3166上标为独立主权": "是"
  },
  "84": {
    "中文名称": "德国",
    "英文短名称": "Germany",
    "二位代码": "DE",
    "三位代码": "DEU",
    "数字代码": "276",
    "ISO.3166-2": "ISO 3166-2:DE",
    "ISO.3166上标为独立主权": "是"
  },
  "85": {
    "中文名称": "加纳",
    "英文短名称": "Ghana",
    "二位代码": "GH",
    "三位代码": "GHA",
    "数字代码": "288",
    "ISO.3166-2": "ISO 3166-2:GH",
    "ISO.3166上标为独立主权": "是"
  },
  "86": {
    "中文名称": "直布罗陀",
    "英文短名称": "Gibraltar",
    "二位代码": "GI",
    "三位代码": "GIB",
    "数字代码": "292",
    "ISO.3166-2": "ISO 3166-2:GI",
    "ISO.3166上标为独立主权": "否"
  },
  "87": {
    "中文名称": "希腊",
    "英文短名称": "Greece",
    "二位代码": "GR",
    "三位代码": "GRC",
    "数字代码": "300",
    "ISO.3166-2": "ISO 3166-2:GR",
    "ISO.3166上标为独立主权": "是"
  },
  "88": {
    "中文名称": "格陵兰",
    "英文短名称": "Greenland",
    "二位代码": "GL",
    "三位代码": "GRL",
    "数字代码": "304",
    "ISO.3166-2": "ISO 3166-2:GL",
    "ISO.3166上标为独立主权": "否"
  },
  "89": {
    "中文名称": "格林纳达",
    "英文短名称": "Grenada",
    "二位代码": "GD",
    "三位代码": "GRD",
    "数字代码": "308",
    "ISO.3166-2": "ISO 3166-2:GD",
    "ISO.3166上标为独立主权": "是"
  },
  "90": {
    "中文名称": "瓜德罗普",
    "英文短名称": "Guadeloupe",
    "二位代码": "GP",
    "三位代码": "GLP",
    "数字代码": "312",
    "ISO.3166-2": "ISO 3166-2:GP",
    "ISO.3166上标为独立主权": "否"
  },
  "91": {
    "中文名称": "关岛",
    "英文短名称": "Guam",
    "二位代码": "GU",
    "三位代码": "GUM",
    "数字代码": "316",
    "ISO.3166-2": "ISO 3166-2:GU",
    "ISO.3166上标为独立主权": "否"
  },
  "92": {
    "中文名称": "危地马拉",
    "英文短名称": "Guatemala",
    "二位代码": "GT",
    "三位代码": "GTM",
    "数字代码": "320",
    "ISO.3166-2": "ISO 3166-2:GT",
    "ISO.3166上标为独立主权": "是"
  },
  "93": {
    "中文名称": "根西",
    "英文短名称": "Guernsey",
    "二位代码": "GG",
    "三位代码": "GGY",
    "数字代码": "831",
    "ISO.3166-2": "ISO 3166-2:GG",
    "ISO.3166上标为独立主权": "否"
  },
  "94": {
    "中文名称": "几内亚",
    "英文短名称": "Guinea",
    "二位代码": "GN",
    "三位代码": "GIN",
    "数字代码": "324",
    "ISO.3166-2": "ISO 3166-2:GN",
    "ISO.3166上标为独立主权": "是"
  },
  "95": {
    "中文名称": "几内亚比绍",
    "英文短名称": "Guinea-Bissau",
    "二位代码": "GW",
    "三位代码": "GNB",
    "数字代码": "624",
    "ISO.3166-2": "ISO 3166-2:GW",
    "ISO.3166上标为独立主权": "是"
  },
  "96": {
    "中文名称": "圭亚那",
    "英文短名称": "Guyana",
    "二位代码": "GY",
    "三位代码": "GUY",
    "数字代码": "328",
    "ISO.3166-2": "ISO 3166-2:GY",
    "ISO.3166上标为独立主权": "是"
  },
  "97": {
    "中文名称": "海地",
    "英文短名称": "Haiti",
    "二位代码": "HT",
    "三位代码": "HTI",
    "数字代码": "332",
    "ISO.3166-2": "ISO 3166-2:HT",
    "ISO.3166上标为独立主权": "是"
  },
  "98": {
    "中文名称": "赫德岛和麦克唐纳群岛",
    "英文短名称": "Heard Island and McDonald Islands",
    "二位代码": "HM",
    "三位代码": "HMD",
    "数字代码": "334",
    "ISO.3166-2": "ISO 3166-2:HM",
    "ISO.3166上标为独立主权": "否"
  },
  "99": {
    "中文名称": "梵蒂冈",
    "英文短名称": "Holy See",
    "二位代码": "VA",
    "三位代码": "VAT",
    "数字代码": "336",
    "ISO.3166-2": "ISO 3166-2:VA",
    "ISO.3166上标为独立主权": "是"
  },
  "100": {
    "中文名称": "洪都拉斯",
    "英文短名称": "Honduras",
    "二位代码": "HN",
    "三位代码": "HND",
    "数字代码": "340",
    "ISO.3166-2": "ISO 3166-2:HN",
    "ISO.3166上标为独立主权": "是"
  },
  "101": {
    "中文名称": "中国香港",
    "英文短名称": "Hong Kong",
    "二位代码": "HK",
    "三位代码": "HKG",
    "数字代码": "344",
    "ISO.3166-2": "ISO 3166-2:HK",
    "ISO.3166上标为独立主权": "否"
  },
  "102": {
    "中文名称": "匈牙利",
    "英文短名称": "Hungary",
    "二位代码": "HU",
    "三位代码": "HUN",
    "数字代码": "348",
    "ISO.3166-2": "ISO 3166-2:HU",
    "ISO.3166上标为独立主权": "是"
  },
  "103": {
    "中文名称": "冰岛",
    "英文短名称": "Iceland",
    "二位代码": "IS",
    "三位代码": "ISL",
    "数字代码": "352",
    "ISO.3166-2": "ISO 3166-2:IS",
    "ISO.3166上标为独立主权": "是"
  },
  "104": {
    "中文名称": "印度",
    "英文短名称": "India",
    "二位代码": "IN",
    "三位代码": "IND",
    "数字代码": "356",
    "ISO.3166-2": "ISO 3166-2:IN",
    "ISO.3166上标为独立主权": "是"
  },
  "105": {
    "中文名称": "印度尼西亚",
    "英文短名称": "Indonesia",
    "二位代码": "ID",
    "三位代码": "IDN",
    "数字代码": "360",
    "ISO.3166-2": "ISO 3166-2:ID",
    "ISO.3166上标为独立主权": "是"
  },
  "106": {
    "中文名称": "伊朗",
    "英文短名称": "Iran (Islamic Republic of)",
    "二位代码": "IR",
    "三位代码": "IRN",
    "数字代码": "364",
    "ISO.3166-2": "ISO 3166-2:IR",
    "ISO.3166上标为独立主权": "是"
  },
  "107": {
    "中文名称": "伊拉克",
    "英文短名称": "Iraq",
    "二位代码": "IQ",
    "三位代码": "IRQ",
    "数字代码": "368",
    "ISO.3166-2": "ISO 3166-2:IQ",
    "ISO.3166上标为独立主权": "是"
  },
  "108": {
    "中文名称": "爱尔兰",
    "英文短名称": "Ireland",
    "二位代码": "IE",
    "三位代码": "IRL",
    "数字代码": "372",
    "ISO.3166-2": "ISO 3166-2:IE",
    "ISO.3166上标为独立主权": "是"
  },
  "109": {
    "中文名称": "马恩岛",
    "英文短名称": "Isle of Man",
    "二位代码": "IM",
    "三位代码": "IMN",
    "数字代码": "833",
    "ISO.3166-2": "ISO 3166-2:IM",
    "ISO.3166上标为独立主权": "否"
  },
  "110": {
    "中文名称": "以色列",
    "英文短名称": "Israel",
    "二位代码": "IL",
    "三位代码": "ISR",
    "数字代码": "376",
    "ISO.3166-2": "ISO 3166-2:IL",
    "ISO.3166上标为独立主权": "是"
  },
  "111": {
    "中文名称": "意大利",
    "英文短名称": "Italy",
    "二位代码": "IT",
    "三位代码": "ITA",
    "数字代码": "380",
    "ISO.3166-2": "ISO 3166-2:IT",
    "ISO.3166上标为独立主权": "是"
  },
  "112": {
    "中文名称": "牙买加",
    "英文短名称": "Jamaica",
    "二位代码": "JM",
    "三位代码": "JAM",
    "数字代码": "388",
    "ISO.3166-2": "ISO 3166-2:JM",
    "ISO.3166上标为独立主权": "是"
  },
  "113": {
    "中文名称": "日本",
    "英文短名称": "Japan",
    "二位代码": "JP",
    "三位代码": "JPN",
    "数字代码": "392",
    "ISO.3166-2": "ISO 3166-2:JP",
    "ISO.3166上标为独立主权": "是"
  },
  "114": {
    "中文名称": "泽西",
    "英文短名称": "Jersey",
    "二位代码": "JE",
    "三位代码": "JEY",
    "数字代码": "832",
    "ISO.3166-2": "ISO 3166-2:JE",
    "ISO.3166上标为独立主权": "否"
  },
  "115": {
    "中文名称": "约旦",
    "英文短名称": "Jordan",
    "二位代码": "JO",
    "三位代码": "JOR",
    "数字代码": "400",
    "ISO.3166-2": "ISO 3166-2:JO",
    "ISO.3166上标为独立主权": "是"
  },
  "116": {
    "中文名称": "哈萨克斯坦",
    "英文短名称": "Kazakhstan",
    "二位代码": "KZ",
    "三位代码": "KAZ",
    "数字代码": "398",
    "ISO.3166-2": "ISO 3166-2:KZ",
    "ISO.3166上标为独立主权": "是"
  },
  "117": {
    "中文名称": "肯尼亚",
    "英文短名称": "Kenya",
    "二位代码": "KE",
    "三位代码": "KEN",
    "数字代码": "404",
    "ISO.3166-2": "ISO 3166-2:KE",
    "ISO.3166上标为独立主权": "是"
  },
  "118": {
    "中文名称": "基里巴斯",
    "英文短名称": "Kiribati",
    "二位代码": "KI",
    "三位代码": "KIR",
    "数字代码": "296",
    "ISO.3166-2": "ISO 3166-2:KI",
    "ISO.3166上标为独立主权": "是"
  },
  "119": {
    "中文名称": "朝鲜",
    "英文短名称": "Korea (Democratic People's Republic of)",
    "二位代码": "KP",
    "三位代码": "PRK",
    "数字代码": "408",
    "ISO.3166-2": "ISO 3166-2:KP",
    "ISO.3166上标为独立主权": "是"
  },
  "120": {
    "中文名称": "韩国",
    "英文短名称": "Korea (Republic of)",
    "二位代码": "KR",
    "三位代码": "KOR",
    "数字代码": "410",
    "ISO.3166-2": "ISO 3166-2:KR",
    "ISO.3166上标为独立主权": "是"
  },
  "121": {
    "中文名称": "科威特",
    "英文短名称": "Kuwait",
    "二位代码": "KW",
    "三位代码": "KWT",
    "数字代码": "414",
    "ISO.3166-2": "ISO 3166-2:KW",
    "ISO.3166上标为独立主权": "是"
  },
  "122": {
    "中文名称": "吉尔吉斯斯坦",
    "英文短名称": "Kyrgyzstan",
    "二位代码": "KG",
    "三位代码": "KGZ",
    "数字代码": "417",
    "ISO.3166-2": "ISO 3166-2:KG",
    "ISO.3166上标为独立主权": "是"
  },
  "123": {
    "中文名称": "老挝",
    "英文短名称": "Lao People's Democratic Republic",
    "二位代码": "LA",
    "三位代码": "LAO",
    "数字代码": "418",
    "ISO.3166-2": "ISO 3166-2:LA",
    "ISO.3166上标为独立主权": "是"
  },
  "124": {
    "中文名称": "拉脱维亚",
    "英文短名称": "Latvia",
    "二位代码": "LV",
    "三位代码": "LVA",
    "数字代码": "428",
    "ISO.3166-2": "ISO 3166-2:LV",
    "ISO.3166上标为独立主权": "是"
  },
  "125": {
    "中文名称": "黎巴嫩",
    "英文短名称": "Lebanon",
    "二位代码": "LB",
    "三位代码": "LBN",
    "数字代码": "422",
    "ISO.3166-2": "ISO 3166-2:LB",
    "ISO.3166上标为独立主权": "是"
  },
  "126": {
    "中文名称": "莱索托",
    "英文短名称": "Lesotho",
    "二位代码": "LS",
    "三位代码": "LSO",
    "数字代码": "426",
    "ISO.3166-2": "ISO 3166-2:LS",
    "ISO.3166上标为独立主权": "是"
  },
  "127": {
    "中文名称": "利比里亚",
    "英文短名称": "Liberia",
    "二位代码": "LR",
    "三位代码": "LBR",
    "数字代码": "430",
    "ISO.3166-2": "ISO 3166-2:LR",
    "ISO.3166上标为独立主权": "是"
  },
  "128": {
    "中文名称": "利比亚",
    "英文短名称": "Libya",
    "二位代码": "LY",
    "三位代码": "LBY",
    "数字代码": "434",
    "ISO.3166-2": "ISO 3166-2:LY",
    "ISO.3166上标为独立主权": "是"
  },
  "129": {
    "中文名称": "列支敦士登",
    "英文短名称": "Liechtenstein",
    "二位代码": "LI",
    "三位代码": "LIE",
    "数字代码": "438",
    "ISO.3166-2": "ISO 3166-2:LI",
    "ISO.3166上标为独立主权": "是"
  },
  "130": {
    "中文名称": "立陶宛",
    "英文短名称": "Lithuania",
    "二位代码": "LT",
    "三位代码": "LTU",
    "数字代码": "440",
    "ISO.3166-2": "ISO 3166-2:LT",
    "ISO.3166上标为独立主权": "是"
  },
  "131": {
    "中文名称": "卢森堡",
    "英文短名称": "Luxembourg",
    "二位代码": "LU",
    "三位代码": "LUX",
    "数字代码": "442",
    "ISO.3166-2": "ISO 3166-2:LU",
    "ISO.3166上标为独立主权": "是"
  },
  "132": {
    "中文名称": "中国澳门",
    "英文短名称": "Macao",
    "二位代码": "MO",
    "三位代码": "MAC",
    "数字代码": "446",
    "ISO.3166-2": "ISO 3166-2:MO",
    "ISO.3166上标为独立主权": "否"
  },
  "133": {
    "中文名称": "马达加斯加",
    "英文短名称": "Madagascar",
    "二位代码": "MG",
    "三位代码": "MDG",
    "数字代码": "450",
    "ISO.3166-2": "ISO 3166-2:MG",
    "ISO.3166上标为独立主权": "是"
  },
  "134": {
    "中文名称": "马拉维",
    "英文短名称": "Malawi",
    "二位代码": "MW",
    "三位代码": "MWI",
    "数字代码": "454",
    "ISO.3166-2": "ISO 3166-2:MW",
    "ISO.3166上标为独立主权": "是"
  },
  "135": {
    "中文名称": "马来西亚",
    "英文短名称": "Malaysia",
    "二位代码": "MY",
    "三位代码": "MYS",
    "数字代码": "458",
    "ISO.3166-2": "ISO 3166-2:MY",
    "ISO.3166上标为独立主权": "是"
  },
  "136": {
    "中文名称": "马尔代夫",
    "英文短名称": "Maldives",
    "二位代码": "MV",
    "三位代码": "MDV",
    "数字代码": "462",
    "ISO.3166-2": "ISO 3166-2:MV",
    "ISO.3166上标为独立主权": "是"
  },
  "137": {
    "中文名称": "马里",
    "英文短名称": "Mali",
    "二位代码": "ML",
    "三位代码": "MLI",
    "数字代码": "466",
    "ISO.3166-2": "ISO 3166-2:ML",
    "ISO.3166上标为独立主权": "是"
  },
  "138": {
    "中文名称": "马耳他",
    "英文短名称": "Malta",
    "二位代码": "MT",
    "三位代码": "MLT",
    "数字代码": "470",
    "ISO.3166-2": "ISO 3166-2:MT",
    "ISO.3166上标为独立主权": "是"
  },
  "139": {
    "中文名称": "马绍尔群岛",
    "英文短名称": "Marshall Islands",
    "二位代码": "MH",
    "三位代码": "MHL",
    "数字代码": "584",
    "ISO.3166-2": "ISO 3166-2:MH",
    "ISO.3166上标为独立主权": "是"
  },
  "140": {
    "中文名称": "马提尼克",
    "英文短名称": "Martinique",
    "二位代码": "MQ",
    "三位代码": "MTQ",
    "数字代码": "474",
    "ISO.3166-2": "ISO 3166-2:MQ",
    "ISO.3166上标为独立主权": "否"
  },
  "141": {
    "中文名称": "毛里塔尼亚",
    "英文短名称": "Mauritania",
    "二位代码": "MR",
    "三位代码": "MRT",
    "数字代码": "478",
    "ISO.3166-2": "ISO 3166-2:MR",
    "ISO.3166上标为独立主权": "是"
  },
  "142": {
    "中文名称": "毛里求斯",
    "英文短名称": "Mauritius",
    "二位代码": "MU",
    "三位代码": "MUS",
    "数字代码": "480",
    "ISO.3166-2": "ISO 3166-2:MU",
    "ISO.3166上标为独立主权": "是"
  },
  "143": {
    "中文名称": "马约特",
    "英文短名称": "Mayotte",
    "二位代码": "YT",
    "三位代码": "MYT",
    "数字代码": "175",
    "ISO.3166-2": "ISO 3166-2:YT",
    "ISO.3166上标为独立主权": "否"
  },
  "144": {
    "中文名称": "墨西哥",
    "英文短名称": "Mexico",
    "二位代码": "MX",
    "三位代码": "MEX",
    "数字代码": "484",
    "ISO.3166-2": "ISO 3166-2:MX",
    "ISO.3166上标为独立主权": "是"
  },
  "145": {
    "中文名称": "密克罗尼西亚联邦",
    "英文短名称": "Micronesia (Federated States of)",
    "二位代码": "FM",
    "三位代码": "FSM",
    "数字代码": "583",
    "ISO.3166-2": "ISO 3166-2:FM",
    "ISO.3166上标为独立主权": "是"
  },
  "146": {
    "中文名称": "摩尔多瓦",
    "英文短名称": "Moldova (Republic of)",
    "二位代码": "MD",
    "三位代码": "MDA",
    "数字代码": "498",
    "ISO.3166-2": "ISO 3166-2:MD",
    "ISO.3166上标为独立主权": "是"
  },
  "147": {
    "中文名称": "摩纳哥",
    "英文短名称": "Monaco",
    "二位代码": "MC",
    "三位代码": "MCO",
    "数字代码": "492",
    "ISO.3166-2": "ISO 3166-2:MC",
    "ISO.3166上标为独立主权": "是"
  },
  "148": {
    "中文名称": "蒙古",
    "英文短名称": "Mongolia",
    "二位代码": "MN",
    "三位代码": "MNG",
    "数字代码": "496",
    "ISO.3166-2": "ISO 3166-2:MN",
    "ISO.3166上标为独立主权": "是"
  },
  "149": {
    "中文名称": "黑山",
    "英文短名称": "Montenegro",
    "二位代码": "ME",
    "三位代码": "MNE",
    "数字代码": "499",
    "ISO.3166-2": "ISO 3166-2:ME",
    "ISO.3166上标为独立主权": "是"
  },
  "150": {
    "中文名称": "蒙特塞拉特",
    "英文短名称": "Montserrat",
    "二位代码": "MS",
    "三位代码": "MSR",
    "数字代码": "500",
    "ISO.3166-2": "ISO 3166-2:MS",
    "ISO.3166上标为独立主权": "否"
  },
  "151": {
    "中文名称": "摩洛哥",
    "英文短名称": "Morocco",
    "二位代码": "MA",
    "三位代码": "MAR",
    "数字代码": "504",
    "ISO.3166-2": "ISO 3166-2:MA",
    "ISO.3166上标为独立主权": "是"
  },
  "152": {
    "中文名称": "莫桑比克",
    "英文短名称": "Mozambique",
    "二位代码": "MZ",
    "三位代码": "MOZ",
    "数字代码": "508",
    "ISO.3166-2": "ISO 3166-2:MZ",
    "ISO.3166上标为独立主权": "是"
  },
  "153": {
    "中文名称": "缅甸",
    "英文短名称": "Myanmar",
    "二位代码": "MM",
    "三位代码": "MMR",
    "数字代码": "104",
    "ISO.3166-2": "ISO 3166-2:MM",
    "ISO.3166上标为独立主权": "是"
  },
  "154": {
    "中文名称": "纳米比亚",
    "英文短名称": "Namibia",
    "二位代码": "NA",
    "三位代码": "NAM",
    "数字代码": "516",
    "ISO.3166-2": "ISO 3166-2:NA",
    "ISO.3166上标为独立主权": "是"
  },
  "155": {
    "中文名称": "瑙鲁",
    "英文短名称": "Nauru",
    "二位代码": "NR",
    "三位代码": "NRU",
    "数字代码": "520",
    "ISO.3166-2": "ISO 3166-2:NR",
    "ISO.3166上标为独立主权": "是"
  },
  "156": {
    "中文名称": "尼泊尔",
    "英文短名称": "Nepal",
    "二位代码": "NP",
    "三位代码": "NPL",
    "数字代码": "524",
    "ISO.3166-2": "ISO 3166-2:NP",
    "ISO.3166上标为独立主权": "是"
  },
  "157": {
    "中文名称": "荷兰",
    "英文短名称": "Netherlands",
    "二位代码": "NL",
    "三位代码": "NLD",
    "数字代码": "528",
    "ISO.3166-2": "ISO 3166-2:NL",
    "ISO.3166上标为独立主权": "是"
  },
  "158": {
    "中文名称": "新喀里多尼亚",
    "英文短名称": "New Caledonia",
    "二位代码": "NC",
    "三位代码": "NCL",
    "数字代码": "540",
    "ISO.3166-2": "ISO 3166-2:NC",
    "ISO.3166上标为独立主权": "否"
  },
  "159": {
    "中文名称": "新西兰",
    "英文短名称": "New Zealand",
    "二位代码": "NZ",
    "三位代码": "NZL",
    "数字代码": "554",
    "ISO.3166-2": "ISO 3166-2:NZ",
    "ISO.3166上标为独立主权": "是"
  },
  "160": {
    "中文名称": "尼加拉瓜",
    "英文短名称": "Nicaragua",
    "二位代码": "NI",
    "三位代码": "NIC",
    "数字代码": "558",
    "ISO.3166-2": "ISO 3166-2:NI",
    "ISO.3166上标为独立主权": "是"
  },
  "161": {
    "中文名称": "尼日尔",
    "英文短名称": "Niger",
    "二位代码": "NE",
    "三位代码": "NER",
    "数字代码": "562",
    "ISO.3166-2": "ISO 3166-2:NE",
    "ISO.3166上标为独立主权": "是"
  },
  "162": {
    "中文名称": "尼日利亚",
    "英文短名称": "Nigeria",
    "二位代码": "NG",
    "三位代码": "NGA",
    "数字代码": "566",
    "ISO.3166-2": "ISO 3166-2:NG",
    "ISO.3166上标为独立主权": "是"
  },
  "163": {
    "中文名称": "纽埃",
    "英文短名称": "Niue",
    "二位代码": "NU",
    "三位代码": "NIU",
    "数字代码": "570",
    "ISO.3166-2": "ISO 3166-2:NU",
    "ISO.3166上标为独立主权": "否"
  },
  "164": {
    "中文名称": "诺福克岛",
    "英文短名称": "Norfolk Island",
    "二位代码": "NF",
    "三位代码": "NFK",
    "数字代码": "574",
    "ISO.3166-2": "ISO 3166-2:NF",
    "ISO.3166上标为独立主权": "否"
  },
  "165": {
    "中文名称": "北马其顿",
    "英文短名称": "North Macedonia",
    "二位代码": "MK",
    "三位代码": "MKD",
    "数字代码": "807",
    "ISO.3166-2": "ISO 3166-2:MK",
    "ISO.3166上标为独立主权": "是"
  },
  "166": {
    "中文名称": "北马里亚纳群岛",
    "英文短名称": "Northern Mariana Islands",
    "二位代码": "MP",
    "三位代码": "MNP",
    "数字代码": "580",
    "ISO.3166-2": "ISO 3166-2:MP",
    "ISO.3166上标为独立主权": "否"
  },
  "167": {
    "中文名称": "挪威",
    "英文短名称": "Norway",
    "二位代码": "NO",
    "三位代码": "NOR",
    "数字代码": "578",
    "ISO.3166-2": "ISO 3166-2:NO",
    "ISO.3166上标为独立主权": "是"
  },
  "168": {
    "中文名称": "阿曼",
    "英文短名称": "Oman",
    "二位代码": "OM",
    "三位代码": "OMN",
    "数字代码": "512",
    "ISO.3166-2": "ISO 3166-2:OM",
    "ISO.3166上标为独立主权": "是"
  },
  "169": {
    "中文名称": "巴基斯坦",
    "英文短名称": "Pakistan",
    "二位代码": "PK",
    "三位代码": "PAK",
    "数字代码": "586",
    "ISO.3166-2": "ISO 3166-2:PK",
    "ISO.3166上标为独立主权": "是"
  },
  "170": {
    "中文名称": "帕劳",
    "英文短名称": "Palau",
    "二位代码": "PW",
    "三位代码": "PLW",
    "数字代码": "585",
    "ISO.3166-2": "ISO 3166-2:PW",
    "ISO.3166上标为独立主权": "是"
  },
  "171": {
    "中文名称": "巴勒斯坦",
    "英文短名称": "Palestine, State of",
    "二位代码": "PS",
    "三位代码": "PSE",
    "数字代码": "275",
    "ISO.3166-2": "ISO 3166-2:PS",
    "ISO.3166上标为独立主权": "否"
  },
  "172": {
    "中文名称": "巴拿马",
    "英文短名称": "Panama",
    "二位代码": "PA",
    "三位代码": "PAN",
    "数字代码": "591",
    "ISO.3166-2": "ISO 3166-2:PA",
    "ISO.3166上标为独立主权": "是"
  },
  "173": {
    "中文名称": "巴布亚新几内亚",
    "英文短名称": "Papua New Guinea",
    "二位代码": "PG",
    "三位代码": "PNG",
    "数字代码": "598",
    "ISO.3166-2": "ISO 3166-2:PG",
    "ISO.3166上标为独立主权": "是"
  },
  "174": {
    "中文名称": "巴拉圭",
    "英文短名称": "Paraguay",
    "二位代码": "PY",
    "三位代码": "PRY",
    "数字代码": "600",
    "ISO.3166-2": "ISO 3166-2:PY",
    "ISO.3166上标为独立主权": "是"
  },
  "175": {
    "中文名称": "秘鲁",
    "英文短名称": "Peru",
    "二位代码": "PE",
    "三位代码": "PER",
    "数字代码": "604",
    "ISO.3166-2": "ISO 3166-2:PE",
    "ISO.3166上标为独立主权": "是"
  },
  "176": {
    "中文名称": "菲律宾",
    "英文短名称": "Philippines",
    "二位代码": "PH",
    "三位代码": "PHL",
    "数字代码": "608",
    "ISO.3166-2": "ISO 3166-2:PH",
    "ISO.3166上标为独立主权": "是"
  },
  "177": {
    "中文名称": "皮特凯恩群岛",
    "英文短名称": "Pitcairn",
    "二位代码": "PN",
    "三位代码": "PCN",
    "数字代码": "612",
    "ISO.3166-2": "ISO 3166-2:PN",
    "ISO.3166上标为独立主权": "否"
  },
  "178": {
    "中文名称": "波兰",
    "英文短名称": "Poland",
    "二位代码": "PL",
    "三位代码": "POL",
    "数字代码": "616",
    "ISO.3166-2": "ISO 3166-2:PL",
    "ISO.3166上标为独立主权": "是"
  },
  "179": {
    "中文名称": "葡萄牙",
    "英文短名称": "Portugal",
    "二位代码": "PT",
    "三位代码": "PRT",
    "数字代码": "620",
    "ISO.3166-2": "ISO 3166-2:PT",
    "ISO.3166上标为独立主权": "是"
  },
  "180": {
    "中文名称": "波多黎各",
    "英文短名称": "Puerto Rico",
    "二位代码": "PR",
    "三位代码": "PRI",
    "数字代码": "630",
    "ISO.3166-2": "ISO 3166-2:PR",
    "ISO.3166上标为独立主权": "否"
  },
  "181": {
    "中文名称": "卡塔尔",
    "英文短名称": "Qatar",
    "二位代码": "QA",
    "三位代码": "QAT",
    "数字代码": "634",
    "ISO.3166-2": "ISO 3166-2:QA",
    "ISO.3166上标为独立主权": "是"
  },
  "182": {
    "中文名称": "留尼汪",
    "英文短名称": "Réunion",
    "二位代码": "RE",
    "三位代码": "REU",
    "数字代码": "638",
    "ISO.3166-2": "ISO 3166-2:RE",
    "ISO.3166上标为独立主权": "否"
  },
  "183": {
    "中文名称": "罗马尼亚",
    "英文短名称": "Romania",
    "二位代码": "RO",
    "三位代码": "ROU",
    "数字代码": "642",
    "ISO.3166-2": "ISO 3166-2:RO",
    "ISO.3166上标为独立主权": "是"
  },
  "184": {
    "中文名称": "俄罗斯",
    "英文短名称": "Russian Federation",
    "二位代码": "RU",
    "三位代码": "RUS",
    "数字代码": "643",
    "ISO.3166-2": "ISO 3166-2:RU",
    "ISO.3166上标为独立主权": "是"
  },
  "185": {
    "中文名称": "卢旺达",
    "英文短名称": "Rwanda",
    "二位代码": "RW",
    "三位代码": "RWA",
    "数字代码": "646",
    "ISO.3166-2": "ISO 3166-2:RW",
    "ISO.3166上标为独立主权": "是"
  },
  "186": {
    "中文名称": "圣巴泰勒米",
    "英文短名称": "Saint Barthélemy",
    "二位代码": "BL",
    "三位代码": "BLM",
    "数字代码": "652",
    "ISO.3166-2": "ISO 3166-2:BL",
    "ISO.3166上标为独立主权": "否"
  },
  "187": {
    "中文名称": "圣赫勒拿、阿森松和特里斯坦-达库尼亚",
    "英文短名称": "Saint Helena, Ascension and Tristan da Cunha",
    "二位代码": "SH",
    "三位代码": "SHN",
    "数字代码": "654",
    "ISO.3166-2": "ISO 3166-2:SH",
    "ISO.3166上标为独立主权": "否"
  },
  "188": {
    "中文名称": "圣基茨和尼维斯",
    "英文短名称": "Saint Kitts and Nevis",
    "二位代码": "KN",
    "三位代码": "KNA",
    "数字代码": "659",
    "ISO.3166-2": "ISO 3166-2:KN",
    "ISO.3166上标为独立主权": "是"
  },
  "189": {
    "中文名称": "圣卢西亚",
    "英文短名称": "Saint Lucia",
    "二位代码": "LC",
    "三位代码": "LCA",
    "数字代码": "662",
    "ISO.3166-2": "ISO 3166-2:LC",
    "ISO.3166上标为独立主权": "是"
  },
  "190": {
    "中文名称": "法属圣马丁",
    "英文短名称": "Saint Martin (French part)",
    "二位代码": "MF",
    "三位代码": "MAF",
    "数字代码": "663",
    "ISO.3166-2": "ISO 3166-2:MF",
    "ISO.3166上标为独立主权": "否"
  },
  "191": {
    "中文名称": "圣皮埃尔和密克隆",
    "英文短名称": "Saint Pierre and Miquelon",
    "二位代码": "PM",
    "三位代码": "SPM",
    "数字代码": "666",
    "ISO.3166-2": "ISO 3166-2:PM",
    "ISO.3166上标为独立主权": "否"
  },
  "192": {
    "中文名称": "圣文森特和格林纳丁斯",
    "英文短名称": "Saint Vincent and the Grenadines",
    "二位代码": "VC",
    "三位代码": "VCT",
    "数字代码": "670",
    "ISO.3166-2": "ISO 3166-2:VC",
    "ISO.3166上标为独立主权": "是"
  },
  "193": {
    "中文名称": "萨摩亚",
    "英文短名称": "Samoa",
    "二位代码": "WS",
    "三位代码": "WSM",
    "数字代码": "882",
    "ISO.3166-2": "ISO 3166-2:WS",
    "ISO.3166上标为独立主权": "是"
  },
  "194": {
    "中文名称": "圣马力诺",
    "英文短名称": "San Marino",
    "二位代码": "SM",
    "三位代码": "SMR",
    "数字代码": "674",
    "ISO.3166-2": "ISO 3166-2:SM",
    "ISO.3166上标为独立主权": "是"
  },
  "195": {
    "中文名称": "圣多美和普林西比",
    "英文短名称": "Sao Tome and Principe",
    "二位代码": "ST",
    "三位代码": "STP",
    "数字代码": "678",
    "ISO.3166-2": "ISO 3166-2:ST",
    "ISO.3166上标为独立主权": "是"
  },
  "196": {
    "中文名称": "沙特阿拉伯",
    "英文短名称": "Saudi Arabia",
    "二位代码": "SA",
    "三位代码": "SAU",
    "数字代码": "682",
    "ISO.3166-2": "ISO 3166-2:SA",
    "ISO.3166上标为独立主权": "是"
  },
  "197": {
    "中文名称": "塞内加尔",
    "英文短名称": "Senegal",
    "二位代码": "SN",
    "三位代码": "SEN",
    "数字代码": "686",
    "ISO.3166-2": "ISO 3166-2:SN",
    "ISO.3166上标为独立主权": "是"
  },
  "198": {
    "中文名称": "塞尔维亚",
    "英文短名称": "Serbia",
    "二位代码": "RS",
    "三位代码": "SRB",
    "数字代码": "688",
    "ISO.3166-2": "ISO 3166-2:RS",
    "ISO.3166上标为独立主权": "是"
  },
  "199": {
    "中文名称": "塞舌尔",
    "英文短名称": "Seychelles",
    "二位代码": "SC",
    "三位代码": "SYC",
    "数字代码": "690",
    "ISO.3166-2": "ISO 3166-2:SC",
    "ISO.3166上标为独立主权": "是"
  },
  "200": {
    "中文名称": "塞拉利昂",
    "英文短名称": "Sierra Leone",
    "二位代码": "SL",
    "三位代码": "SLE",
    "数字代码": "694",
    "ISO.3166-2": "ISO 3166-2:SL",
    "ISO.3166上标为独立主权": "是"
  },
  "201": {
    "中文名称": "新加坡",
    "英文短名称": "Singapore",
    "二位代码": "SG",
    "三位代码": "SGP",
    "数字代码": "702",
    "ISO.3166-2": "ISO 3166-2:SG",
    "ISO.3166上标为独立主权": "是"
  },
  "202": {
    "中文名称": "荷属圣马丁",
    "英文短名称": "Sint Maarten (Dutch part)",
    "二位代码": "SX",
    "三位代码": "SXM",
    "数字代码": "534",
    "ISO.3166-2": "ISO 3166-2:SX",
    "ISO.3166上标为独立主权": "否"
  },
  "203": {
    "中文名称": "斯洛伐克",
    "英文短名称": "Slovakia",
    "二位代码": "SK",
    "三位代码": "SVK",
    "数字代码": "703",
    "ISO.3166-2": "ISO 3166-2:SK",
    "ISO.3166上标为独立主权": "是"
  },
  "204": {
    "中文名称": "斯洛文尼亚",
    "英文短名称": "Slovenia",
    "二位代码": "SI",
    "三位代码": "SVN",
    "数字代码": "705",
    "ISO.3166-2": "ISO 3166-2:SI",
    "ISO.3166上标为独立主权": "是"
  },
  "205": {
    "中文名称": "所罗门群岛",
    "英文短名称": "Solomon Islands",
    "二位代码": "SB",
    "三位代码": "SLB",
    "数字代码": "90",
    "ISO.3166-2": "ISO 3166-2:SB",
    "ISO.3166上标为独立主权": "是"
  },
  "206": {
    "中文名称": "索马里",
    "英文短名称": "Somalia",
    "二位代码": "SO",
    "三位代码": "SOM",
    "数字代码": "706",
    "ISO.3166-2": "ISO 3166-2:SO",
    "ISO.3166上标为独立主权": "是"
  },
  "207": {
    "中文名称": "南非",
    "英文短名称": "South Africa",
    "二位代码": "ZA",
    "三位代码": "ZAF",
    "数字代码": "710",
    "ISO.3166-2": "ISO 3166-2:ZA",
    "ISO.3166上标为独立主权": "是"
  },
  "208": {
    "中文名称": "南乔治亚和南桑威奇群岛",
    "英文短名称": "South Georgia and the South Sandwich Islands",
    "二位代码": "GS",
    "三位代码": "SGS",
    "数字代码": "239",
    "ISO.3166-2": "ISO 3166-2:GS",
    "ISO.3166上标为独立主权": "否"
  },
  "209": {
    "中文名称": "南苏丹",
    "英文短名称": "South Sudan",
    "二位代码": "SS",
    "三位代码": "SSD",
    "数字代码": "728",
    "ISO.3166-2": "ISO 3166-2:SS",
    "ISO.3166上标为独立主权": "是"
  },
  "210": {
    "中文名称": "西班牙",
    "英文短名称": "Spain",
    "二位代码": "ES",
    "三位代码": "ESP",
    "数字代码": "724",
    "ISO.3166-2": "ISO 3166-2:ES",
    "ISO.3166上标为独立主权": "是"
  },
  "211": {
    "中文名称": "斯里兰卡",
    "英文短名称": "Sri Lanka",
    "二位代码": "LK",
    "三位代码": "LKA",
    "数字代码": "144",
    "ISO.3166-2": "ISO 3166-2:LK",
    "ISO.3166上标为独立主权": "是"
  },
  "212": {
    "中文名称": "苏丹",
    "英文短名称": "Sudan",
    "二位代码": "SD",
    "三位代码": "SDN",
    "数字代码": "729",
    "ISO.3166-2": "ISO 3166-2:SD",
    "ISO.3166上标为独立主权": "是"
  },
  "213": {
    "中文名称": "苏里南",
    "英文短名称": "Suriname",
    "二位代码": "SR",
    "三位代码": "SUR",
    "数字代码": "740",
    "ISO.3166-2": "ISO 3166-2:SR",
    "ISO.3166上标为独立主权": "是"
  },
  "214": {
    "中文名称": "斯瓦尔巴和扬马延",
    "英文短名称": "Svalbard and Jan Mayen",
    "二位代码": "SJ",
    "三位代码": "SJM",
    "数字代码": "744",
    "ISO.3166-2": "ISO 3166-2:SJ",
    "ISO.3166上标为独立主权": "否"
  },
  "215": {
    "中文名称": "瑞典",
    "英文短名称": "Sweden",
    "二位代码": "SE",
    "三位代码": "SWE",
    "数字代码": "752",
    "ISO.3166-2": "ISO 3166-2:SE",
    "ISO.3166上标为独立主权": "是"
  },
  "216": {
    "中文名称": "瑞士",
    "英文短名称": "Switzerland",
    "二位代码": "CH",
    "三位代码": "CHE",
    "数字代码": "756",
    "ISO.3166-2": "ISO 3166-2:CH",
    "ISO.3166上标为独立主权": "是"
  },
  "217": {
    "中文名称": "叙利亚",
    "英文短名称": "Syrian Arab Republic",
    "二位代码": "SY",
    "三位代码": "SYR",
    "数字代码": "760",
    "ISO.3166-2": "ISO 3166-2:SY",
    "ISO.3166上标为独立主权": "是"
  },
  "218": {
    "中文名称": "中国台湾省",
    "英文短名称": "Taiwan, Province of China",
    "二位代码": "TW",
    "三位代码": "TWN",
    "数字代码": "158",
    "ISO.3166-2": "ISO 3166-2:TW",
    "ISO.3166上标为独立主权": "否"
  },
  "219": {
    "中文名称": "塔吉克斯坦",
    "英文短名称": "Tajikistan",
    "二位代码": "TJ",
    "三位代码": "TJK",
    "数字代码": "762",
    "ISO.3166-2": "ISO 3166-2:TJ",
    "ISO.3166上标为独立主权": "是"
  },
  "220": {
    "中文名称": "坦桑尼亚",
    "英文短名称": "Tanzania, United Republic of",
    "二位代码": "TZ",
    "三位代码": "TZA",
    "数字代码": "834",
    "ISO.3166-2": "ISO 3166-2:TZ",
    "ISO.3166上标为独立主权": "是"
  },
  "221": {
    "中文名称": "泰国",
    "英文短名称": "Thailand",
    "二位代码": "TH",
    "三位代码": "THA",
    "数字代码": "764",
    "ISO.3166-2": "ISO 3166-2:TH",
    "ISO.3166上标为独立主权": "是"
  },
  "222": {
    "中文名称": "东帝汶",
    "英文短名称": "Timor-Leste",
    "二位代码": "TL",
    "三位代码": "TLS",
    "数字代码": "626",
    "ISO.3166-2": "ISO 3166-2:TL",
    "ISO.3166上标为独立主权": "是"
  },
  "223": {
    "中文名称": "多哥",
    "英文短名称": "Togo",
    "二位代码": "TG",
    "三位代码": "TGO",
    "数字代码": "768",
    "ISO.3166-2": "ISO 3166-2:TG",
    "ISO.3166上标为独立主权": "是"
  },
  "224": {
    "中文名称": "托克劳",
    "英文短名称": "Tokelau",
    "二位代码": "TK",
    "三位代码": "TKL",
    "数字代码": "772",
    "ISO.3166-2": "ISO 3166-2:TK",
    "ISO.3166上标为独立主权": "否"
  },
  "225": {
    "中文名称": "汤加",
    "英文短名称": "Tonga",
    "二位代码": "TO",
    "三位代码": "TON",
    "数字代码": "776",
    "ISO.3166-2": "ISO 3166-2:TO",
    "ISO.3166上标为独立主权": "是"
  },
  "226": {
    "中文名称": "特立尼达和多巴哥",
    "英文短名称": "Trinidad and Tobago",
    "二位代码": "TT",
    "三位代码": "TTO",
    "数字代码": "780",
    "ISO.3166-2": "ISO 3166-2:TT",
    "ISO.3166上标为独立主权": "是"
  },
  "227": {
    "中文名称": "突尼斯",
    "英文短名称": "Tunisia",
    "二位代码": "TN",
    "三位代码": "TUN",
    "数字代码": "788",
    "ISO.3166-2": "ISO 3166-2:TN",
    "ISO.3166上标为独立主权": "是"
  },
  "228": {
    "中文名称": "土耳其",
    "英文短名称": "Türkiye",
    "二位代码": "TR",
    "三位代码": "TUR",
    "数字代码": "792",
    "ISO.3166-2": "ISO 3166-2:TR",
    "ISO.3166上标为独立主权": "是"
  },
  "229": {
    "中文名称": "土库曼斯坦",
    "英文短名称": "Turkmenistan",
    "二位代码": "TM",
    "三位代码": "TKM",
    "数字代码": "795",
    "ISO.3166-2": "ISO 3166-2:TM",
    "ISO.3166上标为独立主权": "是"
  },
  "230": {
    "中文名称": "特克斯和凯科斯群岛",
    "英文短名称": "Turks and Caicos Islands",
    "二位代码": "TC",
    "三位代码": "TCA",
    "数字代码": "796",
    "ISO.3166-2": "ISO 3166-2:TC",
    "ISO.3166上标为独立主权": "否"
  },
  "231": {
    "中文名称": "图瓦卢",
    "英文短名称": "Tuvalu",
    "二位代码": "TV",
    "三位代码": "TUV",
    "数字代码": "798",
    "ISO.3166-2": "ISO 3166-2:TV",
    "ISO.3166上标为独立主权": "是"
  },
  "232": {
    "中文名称": "乌干达",
    "英文短名称": "Uganda",
    "二位代码": "UG",
    "三位代码": "UGA",
    "数字代码": "800",
    "ISO.3166-2": "ISO 3166-2:UG",
    "ISO.3166上标为独立主权": "是"
  },
  "233": {
    "中文名称": "乌克兰",
    "英文短名称": "Ukraine",
    "二位代码": "UA",
    "三位代码": "UKR",
    "数字代码": "804",
    "ISO.3166-2": "ISO 3166-2:UA",
    "ISO.3166上标为独立主权": "是"
  },
  "234": {
    "中文名称": "阿联酋",
    "英文短名称": "United Arab Emirates",
    "二位代码": "AE",
    "三位代码": "ARE",
    "数字代码": "784",
    "ISO.3166-2": "ISO 3166-2:AE",
    "ISO.3166上标为独立主权": "是"
  },
  "235": {
    "中文名称": "英国",
    "英文短名称": "United Kingdom of Great Britain and Northern Ireland",
    "二位代码": "GB",
    "三位代码": "GBR",
    "数字代码": "826",
    "ISO.3166-2": "ISO 3166-2:GB",
    "ISO.3166上标为独立主权": "是"
  },
  "236": {
    "中文名称": "美国",
    "英文短名称": "United States of America",
    "二位代码": "US",
    "三位代码": "USA",
    "数字代码": "840",
    "ISO.3166-2": "ISO 3166-2:US",
    "ISO.3166上标为独立主权": "是"
  },
  "237": {
    "中文名称": "美国本土外小岛屿",
    "英文短名称": "United States Minor Outlying Islands",
    "二位代码": "UM",
    "三位代码": "UMI",
    "数字代码": "581",
    "ISO.3166-2": "ISO 3166-2:UM",
    "ISO.3166上标为独立主权": "否"
  },
  "238": {
    "中文名称": "乌拉圭",
    "英文短名称": "Uruguay",
    "二位代码": "UY",
    "三位代码": "URY",
    "数字代码": "858",
    "ISO.3166-2": "ISO 3166-2:UY",
    "ISO.3166上标为独立主权": "是"
  },
  "239": {
    "中文名称": "乌兹别克斯坦",
    "英文短名称": "Uzbekistan",
    "二位代码": "UZ",
    "三位代码": "UZB",
    "数字代码": "860",
    "ISO.3166-2": "ISO 3166-2:UZ",
    "ISO.3166上标为独立主权": "是"
  },
  "240": {
    "中文名称": "瓦努阿图",
    "英文短名称": "Vanuatu",
    "二位代码": "VU",
    "三位代码": "VUT",
    "数字代码": "548",
    "ISO.3166-2": "ISO 3166-2:VU",
    "ISO.3166上标为独立主权": "是"
  },
  "241": {
    "中文名称": "委内瑞拉",
    "英文短名称": "Venezuela (Bolivarian Republic of)",
    "二位代码": "VE",
    "三位代码": "VEN",
    "数字代码": "862",
    "ISO.3166-2": "ISO 3166-2:VE",
    "ISO.3166上标为独立主权": "是"
  },
  "242": {
    "中文名称": "越南",
    "英文短名称": "Viet Nam",
    "二位代码": "VN",
    "三位代码": "VNM",
    "数字代码": "704",
    "ISO.3166-2": "ISO 3166-2:VN",
    "ISO.3166上标为独立主权": "是"
  },
  "243": {
    "中文名称": "英属维尔京群岛",
    "英文短名称": "Virgin Islands (British)",
    "二位代码": "VG",
    "三位代码": "VGB",
    "数字代码": "92",
    "ISO.3166-2": "ISO 3166-2:VG",
    "ISO.3166上标为独立主权": "否"
  },
  "244": {
    "中文名称": "美属维尔京群岛",
    "英文短名称": "Virgin Islands (U.S.)",
    "二位代码": "VI",
    "三位代码": "VIR",
    "数字代码": "850",
    "ISO.3166-2": "ISO 3166-2:VI",
    "ISO.3166上标为独立主权": "否"
  },
  "245": {
    "中文名称": "瓦利斯和富图纳",
    "英文短名称": "Wallis and Futuna",
    "二位代码": "WF",
    "三位代码": "WLF",
    "数字代码": "876",
    "ISO.3166-2": "ISO 3166-2:WF",
    "ISO.3166上标为独立主权": "否"
  },
  "246": {
    "中文名称": "西撒哈拉",
    "英文短名称": "Western Sahara",
    "二位代码": "EH",
    "三位代码": "ESH",
    "数字代码": "732",
    "ISO.3166-2": "ISO 3166-2:EH",
    "ISO.3166上标为独立主权": "否"
  },
  "247": {
    "中文名称": "也门",
    "英文短名称": "Yemen",
    "二位代码": "YE",
    "三位代码": "YEM",
    "数字代码": "887",
    "ISO.3166-2": "ISO 3166-2:YE",
    "ISO.3166上标为独立主权": "是"
  },
  "248": {
    "中文名称": "赞比亚",
    "英文短名称": "Zambia",
    "二位代码": "ZM",
    "三位代码": "ZMB",
    "数字代码": "894",
    "ISO.3166-2": "ISO 3166-2:ZM",
    "ISO.3166上标为独立主权": "是"
  },
  "249": {
    "中文名称": "津巴布韦",
    "英文短名称": "Zimbabwe",
    "二位代码": "ZW",
    "三位代码": "ZWE",
    "数字代码": "716",
    "ISO.3166-2": "ISO 3166-2:ZW",
    "ISO.3166上标为独立主权": "是"
  }
}

app.config.globalProperties.$getFlagSrc = (code) => {
  return new URL(`./assets/country/${code.toUpperCase()}.svg`, import.meta.url).href;
}

app.use(router).use(Vue3Dragscroll).use(tooltip).mount('#app')
